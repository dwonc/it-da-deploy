...
const useUserChatWebSocket = ({
  ...
  const { updateChatRoom, setNewMessageNotification, increaseTotalUnread } =
    useUserChatStore();

  const subscribeToRoom = useCallback(
    (client: Client, rId: number) => {
          ...
          try {
            ...
            if (data.type === "MESSAGES_READ") {
              ...
              onMessagesReadRef.current?.(
                Number(data.roomId) || rId,
                Number(data.readerId),
              );
              return;
            }
        ...
        client.subscribe(`/topic/message/${userId}`, (message: IMessage) => {
          try {
            ...
            updateChatRoom(messageRoomId, {
              ...
              unreadCount:
                currentRoomId === messageRoomId ? 0 : data.unreadCount,
            });
          } catch (e) {

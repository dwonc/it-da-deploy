...
const ChatMessage: React.FC<Props> = ({ message, isMine, onLocationClick }) => {
  ...
  console.log(
    `🎨 [RENDER] messageId=${message.messageId}, unreadCount=${message.unreadCount}`,
  );
  ...
  const renderSpecialContent = () => {
    ...
    if (message.type === "BILL") {
      if (!parsedData)
        return (
          <div className="loading-placeholder">정산 정보를 불러오는 중...</div>
        );

      const perPerson = Math.floor(
        (parsedData.totalAmount || 0) / (parsedData.participants?.length || 1),
      );
      const participants = parsedData.participants || [];
      const isAllPaid =
        participants.length > 0 && participants.every((p: any) => p.isPaid);
      ...
      const handleCheckPaid = async (participantUserId: number) => {
        ...
        try {
          ...
          await api.post(
            `/social/messages/${Number(realMessageId)}/bill/check`,
            {
      ...
      return (
        <div
          className="bill-calc-card"
          style={{ display: "block", minHeight: "200px", background: "#fff" }}
        >
          <div
            className="bill-calc-header"
            style={{
              display: "flex",
              alignItems: "center",
              color: "#333",
          ...
          <div
            className="bill-calc-summary"
            style={{ padding: "0 10px 15px", borderBottom: "1px solid #eee" }}
          >
            <div
              className="row"
              style={{
            ...
            >
              ...
              <strong style={{ color: "#000" }}>
                {Number(parsedData.totalAmount).toLocaleString()}원
              </strong>
            </div>
            <div
              className="row per"
            ...
            >
              <span>1인당</span>
              <strong style={{ color: "#6c5ce7", fontSize: "1.2rem" }}>
          ...
          >
            {isAllPaid ? (
              <strong style={{ color: "#27ae60", fontSize: "1.05rem" }}>
                🎉 모든 정산이 완료되었습니다!
              </strong>
            ) : (
  ...
  return (
    <div className={`message-item ${isMine ? "mine" : "others"}`}>
      ...
      <div
        ...
        style={{
          width:
            message.type === "POLL" || message.type === "BILL"
              ? "100%"
              : "auto",
          display: "flex",
          flexDirection: "column",
      ...
      >
        {!isMine && (
          <div className="sender-name" style={{ fontWeight: "bold" }}>
        ...
        >
          ...
          <div
            className={`chat-bubble ${isMine ? "mine-bubble" : "others-bubble"}`}
            style={{
              width:
                message.type === "POLL" || message.type === "BILL"
                  ? "100%"

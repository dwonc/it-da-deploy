...
const API_BASE_URL =
  "import.meta.env.VITE_API_URL || 'https://api.it-da.cloud'/api";
const PAGE_SIZE = 20;
    ...
    (set, get) => ({
      ...
      fetchMeetings: async () => {
        ...
        try {
          ...
          console.log(
            `✅ 첫 페이지 로드 완료: ${meetingsData.length}개, 전체: ${totalElements}개, hasMore: ${hasMore}`,
          );
        } catch (error) {
      ...
      fetchMoreMeetings: async () => {
        ...
        try {
          ...
          // ✅ totalCount 또는 totalElements 둘 다 지원
          const totalElements =
            data.totalCount || data.totalElements || get().totalElements;
          ...
          if (newMeetings.length > 0) {
            ...
            console.log(
              `✅ 페이지 ${nextPage} 로드 완료: ${uniqueNew.length}개 추가, 총 ${meetings.length + uniqueNew.length}개`,
            );
          } else {
      ...
      fetchMeetingsByCategory: async (
        ...
        try {
          ...
          console.log(
            `✅ 카테고리 [${category}] 첫 페이지 로드: ${meetingsData.length}개, 전체: ${totalElements}개`,
          );
        } catch (error) {
      ...
      // 카테고리별 추가 페이지 로드
      fetchMoreMeetingsByCategory: async (
        category: string,
        subcategory?: string,
      ) => {
        const { isLoadingMore, hasMore, currentPage, meetings } = get();

  ...
  useEffect(() => {
    ...
    if (storedUser) {
      try {
        ...
        if (parsed.userId) {
          console.log(
            "[UserProfile] localStorage에서 userId 확인:",
            parsed.userId,
          );
      ...
      onConnect: () => {
        ...
        client.subscribe(`/topic/follow/${myUserId}`, (message: IMessage) => {
          try {
            ...
            if (
              data.type === "FOLLOW_REJECTED" &&
              data.fromUserId === targetProfileId
            ) {
              setProfile((prev) =>
    ...
    try {
      if (isFollowingUser()) {
        await apiClient.delete(
          `/api/users/${currentUserId}/follow/${profile.userId}`,
        );
      ...
      if (profile.followRequestStatus === "pending") {
        try {
          await apiClient.delete(
            `/api/users/${currentUserId}/follow-request/${profile.userId}/cancel`,
          );
          setProfile((prev) =>
            prev ? { ...prev, followRequestStatus: "none" } : prev,
          );
      ...
      if (isPublicAccount()) {
        ...
        await apiClient.post(
          `/api/users/${currentUserId}/follow-request/${profile.userId}`,
        );
        setProfile((prev) =>
  ...
  const handleToggleFollowUser = async (targetUserId: number) => {
    ...
    try {
      if (targetIsFollowing) {
        await apiClient.delete(
          `/api/users/${currentUserId}/follow/${targetUserId}`,
        );
        setFollowUsers((prev) =>
          prev.map((u) =>

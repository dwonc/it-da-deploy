...
const UserChatListPage: React.FC = () => {
  ...
  const { chatRooms, setChatRooms, totalUnreadCount, setTotalUnreadCount } =
    useUserChatStore();
      ...
      <div className="chat-list">
        {chatRooms.length === 0 ? (
          <div className="chat-list-empty">
            ...
            <button className="empty-btn" onClick={() => navigate("/mypage")}>
              마이페이지로 이동
            </button>
          </div>
        ) : (
            ...
            >
              <div className="room-avatar">
                {getProfileImageUrl(room.otherProfileImage) ? (
                  <img
                    src={getProfileImageUrl(room.otherProfileImage)!}

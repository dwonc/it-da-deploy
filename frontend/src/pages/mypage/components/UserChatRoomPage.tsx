...
const UserChatRoomPage: React.FC = () => {
    ...
    (message: UserChatMessage) => {
      ...
      if (messageSenderId === currentUserId) {
        ...
        if (currentRoomId && currentUserId) {
          apiClient
            .post(
              `/api/user-chat/room/${currentRoomId}/read?userId=${currentUserId}`,
            )
  ...
  const handleMessagesRead = useCallback((rId: number, readerId: number) => {
    ...
    if (currentUserId !== null && readerIdNum !== currentUserId) {
      ...
      setLocalMessages((prev) => {
        ...
        console.log(
          "📝 업데이트 완료, 내 메시지 개수:",
          updated.filter((m) => Number(m.senderId) === currentUserId).length,
        );
        return updated;
      });
    }
    ...
    const fetchChatRoom = async () => {
      ...
      try {
        const roomResponse = await apiClient.get(
          `/api/user-chat/room/${roomId}?userId=${myUserId}`,
        );
        ...
        const messagesResponse = await apiClient.get(
          `/api/user-chat/room/${roomId}/messages?userId=${myUserId}`,
        );
        ...
        await apiClient.post(
          `/api/user-chat/room/${roomId}/read?userId=${myUserId}`,
        );
      } catch (error) {
  ...
  const handleSend = async () => {
    ...
    try {
      await apiClient.post(
        `/api/user-chat/room/${roomId}/message?userId=${myUserId}`,
        { content },
      );
    } catch (error) {
      ...
      <header className="chat-header">
        ...
        <div className="header-center">
          <div className="header-avatar">
            {getProfileImageUrl(chatRoom?.otherProfileImage) ? (
              <img
                src={getProfileImageUrl(chatRoom?.otherProfileImage)!}
                alt=""
              />
            ) : (
        ...
        <div className="header-actions">
          ...
          <button
            className="header-icon-btn"
            onClick={() =>
              chatRoom && navigate(`/profile/id/${chatRoom.otherUserId}`)
            }
            title="상대방 프로필"
          >
            👤
              ...
              >
                ...
                {!isMine && (
                  <div
                    ...
                    onClick={() =>
                      chatRoom &&
                      navigate(`/profile/id/${chatRoom.otherUserId}`)
                    }
                  >
                    {getProfileImageUrl(msg.senderProfileImage) ? (
                      <img
                        src={getProfileImageUrl(msg.senderProfileImage)!}
                        alt=""
                      />
                    ) : (
                      <div className="avatar-placeholder">
                    ...
                    {isMine && (
                      <div className="message-info mine">
                        ...
                        <span className="message-time">
                          {msg.createdAt.split(" ").pop() || msg.createdAt}
                        </span>
                      </div>
                    )}

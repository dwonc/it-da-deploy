...
const UserProfileById: React.FC = () => {
  ...
  const profileUserId = profileUserIdStr
    ? parseInt(profileUserIdStr, 10)
    : null;
              ...
              if (data.type === "FOLLOW_REJECTED") {
                setProfile((prev) => {
                  if (prev && data.fromUserId === prev.userId) {
                    alert(
                      `${data.fromUsername}님이 팔로우 요청을 거절했습니다.`,
                    );
  ...
  const fetchTabData = useCallback(async () => {
    ...
    try {
      const [upcoming, completed, reviews] = await Promise.all([
        mypageApi
          .getUpcomingMeetings(profileUserId, currentUserId || profileUserId)
          .catch((e) => {
            console.log("upcoming 에러:", e);
            return [];
          }),
        mypageApi
          .getCompletedMeetings(profileUserId, currentUserId || profileUserId)
          .catch((e) => {
            console.log("completed 에러:", e);
            return [];
          }),
        mypageApi
          .getMyReviews(profileUserId, currentUserId || profileUserId)
          .catch((e) => {

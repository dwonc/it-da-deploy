...
import UserSelectionModal from "./components/UserSelectionModal"; // ✅ 추가
...
const UserProfile: React.FC = () => {
  ...
  const getMeetingImageUrl = (imageUrl: string | null | undefined) => {
  ...
  }, [profile?.userId, currentUserId, loadTabData, clearTabData, profile]);
  ...
  const fetchProfile = useCallback(async () => {
    ...
    try {
      ...
      const profileData =
        response.data.type === "single" ? response.data.profile : response.data;
  ...
  useEffect(() => {
    if (profile?.userId && currentUserId && !tabDataLoaded) {
      if (profile.canViewFullProfile) loadTabData(profile.userId, true);
      else clearTabData();
    }
  }, [
    profile?.userId,
  ...
  const handleOpenChat = async () => {
    ...
    try {
      const { canSend, message } = await userChatApi.canSendMessage(
        currentUserId,
        profile.userId
      );
      ...
      const room = await userChatApi.getOrCreateRoom(
        currentUserId,
        profile.userId
      );
    ...
    try {
      ...
      if (profile.followRequestStatus === "pending") {
        try {
          ...
          setProfile((prev) =>
            prev ? { ...prev, followRequestStatus: "none" } : prev
          );
      ...
      if (isPublicAccount()) {
        ...
        setProfile((prev) =>
          prev ? { ...prev, followRequestStatus: "pending" } : prev
        );
  ...
  // ✅ UserSelectionModal에서 사용자 선택 핸들러

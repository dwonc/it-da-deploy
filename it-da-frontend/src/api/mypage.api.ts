...
const mypageApi = {
  ...
  async getPendingReviews(
    ...
    try {
      const response = await apiClient.get(
        `/api/users/${userId}/pending-reviews`,
        {
  ...
  // ✅ 진행 중인 모임 목록 (NEW!)
  async getOngoingMeetings(
    userId: number,
    currentUserId: number,
  ): Promise<MyMeeting[]> {
    try {
      const response = await apiClient.get(
        `/api/users/${userId}/ongoing-meetings`,
        {
          params: { currentUserId },
        },
      );
  ...
  async getUpcomingMeetings(
    ...
    try {
      const response = await apiClient.get(
        `/api/users/${userId}/upcoming-meetings`,
        {
          params: { currentUserId },
        },
      );
  ...
  async getCompletedMeetings(
    ...
    try {
      const response = await apiClient.get(
        `/api/users/${userId}/completed-meetings`,
        {
          params: { currentUserId },
        },
      );
  ...
  async getOrganizedMeetings(userId: number): Promise<OrganizedMeeting[]> {
    try {
      const response = await apiClient.get(
        `/api/users/${userId}/organized-meetings`,
      );
      return response.data || [];
    } catch (error) {
  ...
  async createReview(
    ...
    await apiClient.post(
      `/api/users/${userId}/meetings/${meetingId}/reviews`,
      req,
    );
  },
};
